        ...
        async getSeminarsByStatusCode(statusCode) {
            return await axios.get('/seminars/list/status/' + statusCode).then((response) => {
                ...
                result.forEach(vs => {
                    ...
                    vs.base_info = "ID " + vs.ID + " | " + this.getSeminarFullType(vs.seminar_theme.base_seminar_type, vs.seminar_theme) + " | " + this.getDateInMMDDYYYYFormat(vs.start_date) + " | " + vs.class_room.location.code + "-" + vs.class_room.name;
                });
                return result;
            }, (error) => {
