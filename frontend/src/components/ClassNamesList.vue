<template>
  <div class="container">
    <div class="row mt-2">
      <div class="btn-group">
        <button class="iconBtn" title="Dodaj" @click="$router.push({name: 'ClassNameEdit', query: {id: '', action: 'add' }})">
          <i class="fa fa-user-plus"></i>
        </button>
        <button class="iconBtn" title="Pregledaj" :disabled="table.selectedClassName == null" @click="$router.push({name: 'ClassNameEdit', query: {id: table.selectedClassName.ID, action: 'view' }})">
          <i class="fa fa-user"></i>
        </button>
        <button class="iconBtn" title="Izmeni" :disabled="table.selectedClassName == null" @click="$router.push({name: 'ClassNameEdit', query: {id: table.selectedClassName.ID, action: 'update' }})">
          <i class="fa fa-user-md">
          </i></button>
        <label class="m-1" style="font-size: 1.2em; font-style: italic">Nazivi časova</label>
      </div>
    </div>
    <div class="row mt-2">
      <vue-table-lite
          ref="localTable"
          @row-clicked="selectClassName"
...
export default {
  ...
  setup() {
    ...
    const selectClassName= (rowData) => {
      ...
      table.selectedClassName = rowData;
    }
